...
file_path = 'E:\\R\\python_projects\\b_data\\data\\data_BCH'
...
for file in file_list:
    ...
    for i in range(len(open_list)):
        if (pointer / open_list[i]) < 0.999 or (pointer / open_list[i]) > 1.001:
            if pointer / open_list[i] < 0.999:
                counter_long += Decimal('0.0010')
            
            if pointer / open_list[i] > 1.001:
                counter_short += Decimal('0.0010')
            
            pointer = open_list[i]
            percent_counter += Decimal('0.0010')

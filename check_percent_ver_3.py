...
for j in range(30):
    ...
    balance = balance / BCH_open_list[0]
        ...
        if flag_coin == 'BTC':
            if BTC_percent > ETH_percent or BTC_percent > LTC_percent or BTC_percent > BCH_percent:
                ...
                if BTC_percent > ETH_percent and abs(BTC_percent - ETH_percent) > 0.02 and not flag_calc:
                    ...
                    flag_tmp = 'ETH'
                    pointer = i
                    balance *= BTC_open_list[i] * Decimal(0.999)
                    balance /= ETH_open_list[i] * Decimal(0.999)
                    counter += 1
                    ...
                    pointer = i
                    balance *= BTC_open_list[i] * Decimal(0.999)
                    balance /= LTC_open_list[i] * Decimal(0.999)
                    counter += 1
                    ...
                    pointer = i
                    balance *= BTC_open_list[i] * Decimal(0.999)
                    balance /= BCH_open_list[i] * Decimal(0.999)
                    counter += 1
        elif flag_coin == 'ETH':
    ...
    with open('E:\\R\\python_projects\\b_data\\result_start_BCH.txt', 'a+') as f:
